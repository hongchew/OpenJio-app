...
class AddWalletLimit extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      message: '',
  ...
  async handleAddLimit() {
    ...
    if (this.state.walletLimit === null) {
      this.setState({
        message: 'Wallet Limit field is empty. Unable to set wallet limit.',
      });
      ...
      try {
        ...
        console.log(error);
        this.setState({
          message: 'Setting of Wallet Limit Failed.',
        });
      }
    }
  }
...
export default connect(mapStateToProps, mapDispatchToProps)(AddWalletLimit);
